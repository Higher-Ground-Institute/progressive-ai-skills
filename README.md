...
| [contact-extractor](skills/contact-extractor/) | Scheduled Gmail scan that pulls names, emails, phone numbers, organizations, and titles from headers, forwarded messages, signatures, and body text, then saves them to a deduplicated spreadsheet that stays clean across runs. Contributed by [Adam Ruben](https://github.com/SnackAttack555). | Research & Data |
| [linkedin-connector](skills/linkedin-connector/) | Recurring networking follow-up: scans recent calendar meetings for new external attendees, filters out colleagues and people already presented, looks up LinkedIn profiles, and opens "Connect" tabs in Chrome (auto-closing tabs for people you're already connected to). Contributed by [Adam Ruben](https://github.com/SnackAttack555). | Field & Organizing |
| [fecfile](skills/fecfile/) | Analyze FEC (Federal Election Commission) campaign finance filings — contributions, disbursements, loans, debts, independent expenditures — with an opinionated workflow (summary-first, schedule pre-filtering, pandas post-filtering, JSONL streaming) that scales from a small state party report to a presidential campaign filing with hundreds of thousands of itemizations. **External skill** — install from [hodgesmr/agent-fecfile](https://github.com/hodgesmr/agent-fecfile) (MIT, by Matt Hodges); this folder is documentation only. | Research & Data |
